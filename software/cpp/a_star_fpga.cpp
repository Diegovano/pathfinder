...
int main() {
    ...
    std::map<std::pair<float, long long>, Edge> edge_dict;
    ...
    std::priority_queue<std::pair<float, long long>, std::vector<std::pair<float, long long>>, std::greater<std::pair<float, long>>> queue;
    ...
    while(!queue.empty()) {
        ...
        for (int neighbour : cn_neighbours) {
            ...
            Edge edge = edge_dict[std::make_pair(current_node, neighbour)];
            ...
            if (edge.getLength() != 0) {
                ...
                if (g_value.find(neighbour) == g_value.end() || temp_g_score < g_value[neighbour]) {
                    ...
                    queue.push(std::make_pair(f_value[neighbour], neighbour));
                }
            }
    ...
    exec_t2 = clock();
    char buf[50];
    snprintf(buf, sizeof(buf), "%.2f", (double)(exec_t2 - exec_t1));
